...
public class BeanDefinitionParserDelegate {
	...
	public BeanDefinitionHolder parseBeanDefinitionElement(Element ele, @Nullable BeanDefinition containingBean) {
		...
		//设置ID为BeanName
		String beanName = id;
		//如果没有是指定ID，只指定了别名，则从别名列表中取第一个。当作BeanName,并将这个别名删除
		...
		if (beanDefinition != null) {
			//如果ID，Name都没有指定。则自动生成BeanName，
			//委托BeanDefinitionReaderUtils去做
			if (!StringUtils.hasText(beanName)) {
				try {
					if (containingBean != null) {
						...
						//默认实现还是委托BeanDefinitionReaderUtils去做
	...
	public AbstractBeanDefinition parseBeanDefinitionElement(
		...
		//TODO 没明白这个的用意，根据上个方法分析，这里的BeanName可能为Null
		...
		try {
			//根据className和parent创建BeanDefinition
			...
			//解析property元素 //如果有子标签则递归调用parseBeanDefinitionElement(Element,BeanDefinition)
